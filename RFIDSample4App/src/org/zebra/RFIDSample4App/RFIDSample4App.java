...
//java -Xdebug -Xrunjdwp:transport=dt_socket,address=8998,server=y -Djava.library.path=/platform/lib/ -cp .:/
//platform/lib/Symbol.RFID.API3.jar:/apps/lib/apache-httpcomponents-httpcore.jar:/apps/lib/httpclient-4.0.3.jar org.zebra.RFIDSample4
//App.RFIDSample4App
...
public class RFIDSample4App {
	...
	//HTTP
	public static boolean HTTP_OUTPUT_ENABLED = false;
	...
	public void Createmenu()
    {
        ...
        while (keepWorking)
        {
            ...
            System.out.println("------------------------------");
            System.out.println("Built by {N.DZL}, v1.3 20/11/2018");
            System.out.println("------------------------------");
	...
	private void InventoryMenu()
	{
        ...
        while (keepworking)
        {
            ...
            System.out.println("9. Pump data to http server");
	...
	private void SimpleInventory(int submenu) throws InterruptedException,InvalidUsageException,OperationFailureException
	{
			
		if(submenu==1)
			RFIDSample4App.HTTP_OUTPUT_ENABLED=true;
	...
	private void PeriodicInventory(int submenu) throws InterruptedException,InvalidUsageException,OperationFailureException
	{
		...
		{  
	        ...
	        //N.DZL-singulation control
	        //https://developer.zebra.com/thread/35441
	        Antennas antennas = myReader.Config.Antennas;
	        ...
	        for (int i = 0; i < antennas_ixs.length; i++) {
	        	...
	        	Antennas.SingulationControl singulationControl = myReader.Config.Antennas.getSingulationControl(antennaID);
	        	singulationControl.setSession(SESSION.SESSION_S3);    //TAG00002:  USARE S0 O S3
	        	singulationControl.setTagPopulation((short)333);
	        	...
	        	singulationControl.setTagTransitTime((short)100);
	        		        	
	        	myReader.Config.Antennas.setSingulationControl(antennaID, singulationControl);
	        }
		...
		if (myTags != null)
		{
				 if(!isAccess)
				 {
					 for (int index = 0; index < myTags.length; index++) 
					 {
						 ...
						 {
							//tagStore.put(key,totalTags); //rimuovere commento per conteggio singoli tag
							postInfoMessage("ReadTag "+key); 
							postToHttp(FX_reader_name+""+/*tag_time.ConvertTimetoString()+*/",id:"+key+",ant:"+tag_antenna+",rssi:"+tag_RSSI+",pc:"+tag_PC+""+"%0D%0A");
							//uniqueTags++;
						  }
						 totalTags++;
					 }
    ...
    static void postToHttp(String msg)
    {
    	if(RFIDSample4App.HTTP_OUTPUT_ENABLED)
    		httpNetRequest(msg);
    }
    
    static void httpNetRequest(String msg){
    	URL url;
    	
    	try {
			//url = new URL("http://192.168.188.188:8080/helloworld/ReadPostGet.jsp?tag="+msg);
			url = new URL("http://192.168.188.188:8080/helloworld/ReadGetNoResp.jsp?tag="+msg);
			...
			in.close();
			//postInfoMessage(response.toString());
    ...
    static String execLinuxCmd(String cmd) {
	...
	static String FX_reader_name="";
	
	public static void main(String[] args) throws InterruptedException {
		
		String[] cmdsplit = ((String)execLinuxCmd("uname -a")).split(" ");
		if(cmdsplit.length>2){
			FX_reader_name = cmdsplit[1];
			postInfoMessage("Hello by " +FX_reader_name);
		}
